...
// Renders everything about a single job: status header, summary, full
// transcript, topics, plus the reanalyze and delete actions. Used both by the
// history detail view and the quick-transcribe result section so the two stay
// in sync (previously they had drifted — e.g. one had dropped topic transcript
// while the other still rendered it).
export function JobResultPanel({ jobId, onDeleted }: { jobId: string; onDeleted: () => void }) {
  ...
  if (jobQuery.error) {
    return <p className="text-xs text-error">{jobQuery.error.message}</p>;
  }
  if (!job) {
    return <p className="text-xs text-text-secondary">見つかりませんでした</p>;
  }
