...
func main() {
	...
	flag.StringVar(&whisperURL, "whisper-url", envOrDefault("WHISPER_BASE_URL", "https://api.openai.com"), "Whisper API base URL")
	flag.StringVar(&whisperKey, "whisper-key", os.Getenv("WHISPER_API_KEY"), "Whisper API key")
	flag.StringVar(&model, "model", envOrDefault("WHISPER_MODEL", "whisper-1"), "Whisper model name")
	...
	if whisperKey == "" {
		fmt.Fprintln(os.Stderr, "error: Whisper API key is required (--whisper-key or WHISPER_API_KEY)")
		os.Exit(1)
	}
	if geminiKey == "" {
		fmt.Fprintln(os.Stderr, "error: Gemini API key is required (--gemini-key or GEMINI_API_KEY)")
		os.Exit(1)
	}
